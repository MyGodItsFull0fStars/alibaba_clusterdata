...
config_path: str = ''
if parser_arguments.config_file is None or len(parser_arguments.config_file) == 0:
    config_path = instance_yaml_path
else:
    config_path = parser_arguments.config_file
...
print('load datasets')
dataset = UtilizationDataset(is_training=True, small_df=small_df,
                             include_tasks=include_tasks, include_instance=include_instance)
test_set = UtilizationDataset(is_training=False, small_df=small_df,
                              include_tasks=include_tasks, include_instance=include_instance)

print(f'Feature Set shape: {dataset.X.shape}')
print(f'Label Set shape: {dataset.y.shape}')
...
INCLUDE_WANDB: bool = parser_arguments.include_wandb
...
def get_criterion_from_config() -> Union[nn.MSELoss, PenaltyMSELoss, RMSELoss]:
    if 'loss' not in yaml_config['model'].keys():
        return nn.MSELoss().to(device)
    
    criterion_config = yaml_config['model']['loss']
    if criterion_config == 'MSE':
        return nn.MSELoss().to(device)
    elif criterion_config == 'Penalty':
        return PenaltyMSELoss().to(device)
    elif criterion_config == 'RMSE':
...
criterion = get_criterion_from_config()
print(f'criterion: {criterion._get_name()}')
# optimizer function
...
def reorder_dataset(dataset: ForecastDataset, batch_size: int):
    batch_order = np.array([batch for batch in range(
        0, len(dataset), batch_size)], dtype=np.int32)
    ...
    for batch in batch_order:
        if batch >= len(dataset) - (batch_size - 1):
            continue
        filled_batch_order = np.arange(
            batch, batch + batch_size, dtype=np.int32)
...
def get_batch_size_ranges(split_size: int = 5) -> list:
    ...
    bs_sizes = [x for x in range(
        step_size, batch_size, step_size)] + [batch_size]
...
if parser_arguments.dry_run == True:
    ...
    model.eval()
    # tests if the model is able to forward the feature set and exits execution after one forward pass
    for _, (inputs, labels) in enumerate(tqdm(mini_train_loader, leave=False)):
        inputs, labels = inputs.to(device), labels.to(device)
...
def get_combined_data_df(data_set: UtilizationDataset, save_to_file: bool = True, is_training: bool = True) -> pd.DataFrame:
    ...
    def get_prediction_df() -> pd.DataFrame:
        ...
        prediction_df = data_set.y_scaler.inverse_normalization_df(
            prediction_df)
    ...
    def get_actual_data_df() -> pd.DataFrame:
        ...
        actual_data_df = data_set.y_scaler.inverse_normalization_df(
            actual_data_df)
    ...
    combined_df = pd.merge(actual_data_df, prediction_df,
