...
print('load datasets')
dataset = UtilizationDataset(is_training=True, small_df=small_df, include_tasks=False, include_instance=False)
test_set = UtilizationDataset(is_training=False, small_df=small_df, include_tasks=False, include_instance=False)
...
if yaml_config['model']['save_model']:
    model_name = f'models/small-epochs-{num_epochs}-{current_time}'
    torch.save(
        {
            'epoch': num_epochs,
