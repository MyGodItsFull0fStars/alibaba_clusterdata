...
class EvaluationMetrics:
    ...
    def get_all_metrics(actual: pd.Series, predicted: pd.Series, table_name: str = 'metrics') -> pd.DataFrame:
        ...
        over_allocation: float = EvaluationUtils.get_over_allocated_df_percentage(temp_df, 0, 1) * 100
...
class EvaluationUtils:
    ...
    def get_over_allocated_series(df: pd.DataFrame, actual_col, predicted_col) -> pd.Series:
        oa_df = EvaluationUtils.get_over_allocated_df(df, actual_col, predicted_col)
        return oa_df.iloc[:, 1]
